  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: 24 }}>
      ...
      <Card style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
            {userInfo?.avatar ? (
              <img src={coverUrl(userInfo)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ) : (
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}><StatCard icon={<FolderOpenOutlined />} label="作品" value={totalWorks} /></Col>
        <Col span={6}><StatCard icon={<StarOutlined />} label="获赞" value={0} /></Col>
        <Col span={6}><StatCard icon={<BookOutlined />} label="课程" value={0} /></Col>
        <Col span={6}><StatCard icon={<RocketOutlined />} label="学习天数" value={0} /></Col>
      </Row>

      {/* 快捷创作 */}
      <Card title="快捷创作" size="small" style={{ marginBottom: 16 }}>
        <Row gutter={12}>
          <Col span={6}><Button block icon={<PlusOutlined />} onClick={() => window.open('/scratch3/index.html?scene=create', '_blank')}>Scratch</Button></Col>
          <Col span={6}><Button block icon={<PlusOutlined />} onClick={() => window.open('/ide?workType=4', '_blank')}>Python</Button></Col>
          <Col span={6}><Button block icon={<PlusOutlined />} onClick={() => window.open('/scratchjr/home.html', '_blank')}>ScratchJr</Button></Col>
          <Col span={6}><Button block icon={<PlusOutlined />} onClick={() => window.open('/blockly/index.html?lang=zh-hans&scene=create', '_blank')}>Blockly</Button></Col>
        </Row>
      </Card>
