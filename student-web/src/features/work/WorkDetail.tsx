...
function playerContent(w: WorkVO, refCallback?: (el: HTMLIFrameElement | null) => void): React.ReactNode {
  ...
  return <iframe ref={refCallback} src={`/scratch3/player.html?workId=${w.id}`} scrolling="no" title="player"
        ...
        <Col {...RESPONSIVE.colSidebar}>
          <Card styles={{ body: { padding: 0 } }} style={{ overflow: 'hidden' }}>
            <div style={{ position: 'relative', width: '100%', paddingBottom: '75%' }}>
              {playerContent(w, iframeCallback)}
            </div>
          </Card>
          {isMobile && Number(w.workType) !== 3 && (
