...
import { putAction, getAction } from '@/api/client'
import { Space, Modal, Input, message, Popconfirm, Tooltip } from 'antd'

interface Depart { orgCode: string; departName: string }
...
export default function UserList() {
  ...
  useEffect(() => {
    getAction<Depart[]>('/sys/sysDepart/listAll')
      ...
      .then(list => setDepartOptions(list.map(d => ({ label: d.departName, value: d.orgCode }))))
      .catch(() => {})
  }, [])
  ...
  return <>
    <CrudList
      ...
      fields={[{name:'username',label:'账号',required:true},{name:'realname',label:'姓名',required:true},{name:'password',label:'密码'},{name:'phone',label:'手机'},{name:'email',label:'邮箱'},{name:'sex',label:'性别',type:'select',options:[{label:'男',value:1},{label:'女',value:2}]},{name:'orgCode',label:'部门',type:'select',options:departOptions},{name:'status',label:'状态',type:'select',options:[{label:'正常',value:1},{label:'冻结',value:2}]}]}
      onSave={async (vals, record) => record ? api.edit({ ...record, ...vals }) : api.add(vals)}
      extraActions={(record:any)=> (
        <Space>
          {record.username !== 'admin' && <Popconfirm title={record.status===1?'确认冻结?':'确认解冻?'} onConfirm={()=>frozen(record)}><Tooltip title={record.status===1?'冻结':'解冻'}><a><i className="fas fa-snowflake" style={{color: record.status===1?'#1890ff':'#faad14'}} /></a></Tooltip></Popconfirm>}
          <Tooltip title="改密"><a onClick={()=>{ setPwdUser(record); setPwd('') }}><i className="fas fa-key" /></a></Tooltip>
        </Space>
      )}
    />
