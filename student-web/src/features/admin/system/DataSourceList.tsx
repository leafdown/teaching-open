...
export default function Page() {
  ...
  const dbTypeOptions = useDictOptions('db_type')
  const fields = [
    { name: 'code', label: '编码', required: true },
    { name: 'name', label: '名称', required: true },
    { name: 'dbType', label: '数据库类型', required: true, type: 'select' as const, options: toOptions(dbTypeOptions.data) },
    { name: 'dbDriver', label: '驱动' },
    { name: 'dbUrl', label: 'URL', required: true },
    { name: 'dbName', label: '库名' },
    { name: 'dbUsername', label: '用户名' },
    { name: 'dbPassword', label: '密码' },
  ]
  return <CrudList
